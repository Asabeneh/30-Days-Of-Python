...
[← Previous lesson](../day_003_types_and_parsing/day_003_types_and_parsing.md) · [README](../README.md) · [Setup](../SETUP.md) · [VS Code](../VS_CODE_SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../day_005_branching_and_triage/day_005_branching_and_triage.md)
...
1. What is Python Operators, Comparisons, and Decisions? Answer in one sentence.
2. Write one expression using `+`, one using `-`, one using `*`, and one using `/`. Predict and then print all four results.
3. Calculate `17 // 5` and `17 % 5`. Explain what each answer means in the language of complete groups and leftover records.
4. Start with `processed = 0` and use `+=` three times. Print the final value and explain each change.
5. Create variables `new_score = 95` and `previous_best = 88`. Store `new_score > previous_best` in `compare_scores` and assert that it is `True`.
6. Test the difference between `severity > 7` and `severity >= 7` for the values `6`, `7`, and `8`.
7. Create `meets_requirements` that is true only when experience is at least `3` and score is at least `85`.
8. Create `is_auth_event` that is true when an event is either `login_failed` or `access_denied`.
9. Use `not` to create `has_source` from a Boolean called `source_is_missing`.
10. Use `in` to check whether a synthetic event appears in a local set of two allowed event names.
11. Explain in your own words why `==` and `is` answer different questions. Demonstrate with `None` and two equal strings.
12. Evaluate `10 + 2 * 3` and `(10 + 2) * 3`. Explain why the answers differ.
13. Repair a broken triage expression that uses `or` when the written policy requires high severity, a non-empty source, and a known event.
14. Apply the rule to the three-record fixture and print only `record_number` and `label`.
15. Add a boundary test for severity `6`, `7`, and `8`. Explain why the boundary test is more informative than testing only `10`.
16. Safety question: explain why a correct Boolean expression does not authorize scanning, connecting to, uploading to, or collecting data from a real system.
...
[← Day 3](../day_003_types_and_parsing/day_003_types_and_parsing.md) · [Day index](../DAY_INDEX.md) · [Day 5 →](../day_005_branching_and_triage/day_005_branching_and_triage.md)
