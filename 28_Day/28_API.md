...
Application Programming Interface(API). The kind of API I will cover in this section is going to be Web APIs.
...
Web APIs have been moving away from Simple Object Access Protocol (SOAP) based web services and service-oriented architecture (SOA) towards more direct representational state transfer (REST) style web resources.
...
HTTP uses client-server model. An HTTP client opens a connection and sends a request message to an HTTP server and the HTTP server returns message which is the requested resources. When the request response cycle is finished, the server closes the connection.

![HTTP request response cycle](../images/http_request_response_cycle.png)

The format of the request and response messages are similar. Both types of messages have:
...
Let's check out an example of request and response messages by navigating to this site: https://thirtydaysofpython-v1-final.herokuapp.com/
...
- the version of HTTP being used. E.g. GET / HTTP/1.1
-

GET is the most common HTTP method to get or read resources. POST is another common request method, used to create resources.
...
500 Server Error
A complete list of HTTP status codex can be found [here](https://httpstatuses.com/)

### Header Fields

As you have seen in the screenshot above, header lines provide information about the request or response, or about the object sent in the message body.
...
The GET, POST, PUT and DELETE are the HTTP request methods which we are going to implement in an API or a CRUD operation application.
...
3. PUT: Replaces all current representations of the target resource with the uploaded content. We use it modify or update data.
