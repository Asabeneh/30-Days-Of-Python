...
- [Worked examples](#worked-examples)
  ...
  - [Example 3: Check failure explicitly](#example-3-check-failure-explicitly)
  - [Example 4: Set a working directory](#example-4-set-a-working-directory)
  - [Example 5: Use a timeout](#example-5-use-a-timeout)
...
The first runnable example introduces **Processes and Safe System Calls**. Copy it into a new file and run it before changing anything. Then use this table to read the same code slowly. A line-by-line explanation does not replace practice: it shows you what to look for when a program behaves differently from your prediction.
...
| 3 | `result = subprocess.run(` | Assignment: Python evaluates the right side and stores the result under the name on the left. |
| 4 | `["python", "--version"], capture_output=True, text=True, check=False` | Assignment: Python evaluates the right side and stores the result under the name on the left. |
| 5 | `)` | Expression or data declaration: read the names, values, and operators and predict the result. |
| 6 | `print(result.returncode)` | Output call: Python evaluates the argument and writes a representation to the terminal. |
...
1. Run the starter for **Day 31: Processes and Safe System Calls** unchanged. What does it print, return, or create? Record the command and observed result.
