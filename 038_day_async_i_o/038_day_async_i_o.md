# Day 38: Async I/O and Bounded Collection

[Previous](../037_day_processes__threads__and_queues/037_day_processes__threads__and_queues.md) | [Next](../039_day_host_inventories/039_day_host_inventories.md)
...
Complete Day 37, keep [SETUP.md](../SETUP.md) available, and read [SAFETY_AND_LAB_RULES.md](../SAFETY_AND_LAB_RULES.md).
...
Async functions pause at `await` while the event loop can run other tasks. That is useful for waiting on I/O, not for making CPU-heavy work disappear.
...
Use cancellation, deadlines, bounded concurrency, and explicit result collection. An async function does not run until it is awaited or scheduled.

Security connection: a high-concurrency collector can create load quickly. The limit is part of the safety design.
...
> Asynchronous I/O can wait efficiently, but it still needs limits, cancellation, and clear ownership of results.
