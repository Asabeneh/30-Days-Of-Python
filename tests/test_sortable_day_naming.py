...
def test_lessons_use_zero_padded_sortable_names() -> None:
    directories = lesson_directories()
    assert len(directories) == 120
    assert [int(path.name.split("_", 2)[1]) for path in directories] == list(
        range(1, 121)
    )
    assert not list(ROOT.glob("day_[1-9]_*"))
    assert all(
        path.name.split("_", 2)[1] == f"{int(path.name.split('_', 2)[1]):03d}"
...
def test_course_modules_use_zero_padded_names() -> None:
    modules = sorted(
    ...
    assert modules
    assert all(
        path.stem.removeprefix("day").isdigit()
        and len(path.stem.removeprefix("day")) == 3
        for path in modules
    )
